...
- 240797e: `deploy init --target github-pages` が生成する workflow の build ジョブに `pages: read` を付与。`actions/configure-pages` は build ジョブで `GET /repos/{owner}/{repo}/pages` を叩くため、`contents: read` だけだと `Resource not accessible by integration` で 403 になり、build が落ちて deploy が skip される環境があった。書き込み権限と OIDC トークンは deploy ジョブのままなので最小権限は保たれる。
