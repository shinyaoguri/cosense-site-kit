...
- **ゼロから動くサイトまで 1 コマンド**。`npm create cosense-site my-site --theme <id>` で、テーマ入りのそのまま立ち上がるサイトが生成されます。

## クイックスタート

```bash
# ブラウザだけ: GitHub で cosense-theme-default を「Use this template」
# CLI: テーマを選んで取得（default / lab / 任意の user/repo）
npm create cosense-site my-site --theme default
cd my-site
...
`--theme` を省略すると対話でディレクトリとテーマを選べます（`--template` は旧名のエイリアスとして当面残します）。テーマのコードは取得先のリポジトリに同梱（vendored）されているので、取得後はそのまま自分のものとして編集できます。
...
theme-default は本体同梱（npm）の「ライブラリ型」テーマです。**それ以外のテーマは「テーマ＝ Use this template リポジトリ」として配布**します（下記）。

### テーマを選ぶ（Use this template）

ブラウザだけで新しいサイトを作れます。各テーマは「そのテーマを組み込んだ動くサイト」のリポジトリで、**テーマのソースは同梱（vendored）され、自分のものとして自由に編集できます**。npm 依存はフレームワーク（`@cosense-site-kit/*`）だけ。

| テーマ | id | リポジトリ | 用途 |
|---|---|---|---|
| Default | `default` | [cosense-theme-default](https://github.com/shinyaoguri/cosense-theme-default) | 汎用・ドキュメント・個人サイト |
| Lab | `lab` | [cosense-theme-lab](https://github.com/shinyaoguri/cosense-theme-lab) | メンバー / 研究テーマ / 業績 / お知らせ |

CLI から取得する場合は `npm create cosense-site my-site --theme <id>`。

### テーマを配布する

テーマは **「フレームワークに依存し、`@cosense-site-kit/theme-utils` で本文を描画する Astro Integration」** です。配布は **テーマリポジトリ（テーマソースを同梱した Use this template リポジトリ）が基本**で、[cosense-theme-lab](https://github.com/shinyaoguri/cosense-theme-lab) がその参考実装です。作り方は [docs/THEMES.md](./docs/THEMES.md)。発見はゆるく運用 — 既知のテーマをここにリンクします（PR 歓迎）。

> 第三者のテーマは `npm create cosense-site my-site --theme <user/repo>` でも取得できます（「Use this template」と同じリポジトリを CLI で fetch するだけ）。
...
| `create-cosense-site` | テーマ取得（`npm create cosense-site <dir> --theme <id\|user/repo>` → テーマリポジトリを fetch） |
