...
| `#no-date` | このページの公開日・更新日を**表示しない**（About など日付不要なページ向け）。日付データ自体は残るので並び順・フィード・sitemap には引き続き使われる |
...
**日付を表示したくないページ**（About など）には `#no-date` を付けます。ページ上部とホーム/アーカイブ一覧の日付表示が消えます。日付データ自体は残るので、並び順・フィード・sitemap には引き続き使われます。
