...
`cosense-site`（`@cosense-site-kit/cli`）はデータ取得と診断のための CLI です。**ふだん直接叩く必要はほぼありません** — テンプレートの `npm run dev` / `npm run build` がビルド時に自動で Cosense からデータを取得します（`fetch` ステップ／Astro Integration 経由）。直接よく使うのは公開前チェックの `doctor` くらいです。

```
cosense-site fetch  [--force]     Cosense からページ取得 → .cosense-cache/（差分 fetch）
                    [--export <file>]  中間モデル全体を JSON に書き出し
cosense-site doctor [--force]     公開前診断: publish 結果・リンク切れ・構造参照（CI ゲート向け）
...
> `init` / `deploy init` は、テンプレート（[cosense-theme-default](https://github.com/shinyaoguri/cosense-theme-default) を degit）が config と workflow を既に同梱しているため、**テンプレ運用では不要**です。スクラッチや既存の Astro プロジェクトに後付けする場合・配信先を切り替える場合の補助コマンドです。
