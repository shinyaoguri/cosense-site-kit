...
| `@cosense-site-kit/theme-portfolio` | ポートフォリオ向け。`page` / `profile` / `cv` / `blog-post` + ブログ自動振り分け。shinyaoguri.com のデザインを素 CSS で再現 | 個人サイト、研究者ページ、CV 付きポートフォリオ |
| `@cosense-site-kit/theme-lab` (叩き台) | 大学研究室向け。`members` (code:members.yaml) / `publications` (code:publications.yaml) / `research` + `news` のタグ自動振り分け | 研究室サイト、共同研究プロジェクトサイト |
| `@cosense-site-kit/theme-organisation` (叩き台) | 企業・組織向け。Hero + `services` / `cases` / `news` のタグ自動振り分け、多カラムフッタ | コーポレートサイト、サービス紹介サイト |

各テーマの Cosense 側ページ構成と `astro.config.ts` の書き方は、それぞれの README を参照:

- [packages/theme-default/](./packages/theme-default/)
- [packages/theme-portfolio/](./packages/theme-portfolio/) (詳細は [docs/THEMES.md](./docs/THEMES.md))
- [packages/theme-lab/README.md](./packages/theme-lab/README.md)
- [packages/theme-organisation/README.md](./packages/theme-organisation/README.md)
...
| `@cosense-site-kit/theme-lab` | 研究室向けテーマ (メンバー / 業績 / 研究 / ニュース) |
| `@cosense-site-kit/theme-organisation` | 企業・組織向けテーマ (Hero / Services / Cases / News) |
