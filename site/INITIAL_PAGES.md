...
[* できること]
 ...
 clone 不要・ブラウザだけでセットアップできる（テンプレートの「Use this template」）
...
[* 始める]
 clone 不要で始めるなら、テンプレート [https://github.com/shinyaoguri/cosense-theme-default cosense-theme-default] の「Use this template」から。手順の詳細は [Quick start]。
...
```
cosense-site-kit でサイトを立ち上げる手順。clone もターミナルも不要のブラウザ完結ルートと、ローカルで動かすルートの2通りがあります。

[** ルート A: ブラウザだけで作る（おすすめ・clone 不要）]
 1. テンプレート [https://github.com/shinyaoguri/cosense-theme-default cosense-theme-default] を開き、緑の「Use this template」→「Create a new repository」で自分のリポジトリを作成
 2. GitHub の Web エディタで cosense.config.ts を開き、次の3項目だけ編集
  source.project … 自分の公開 Cosense プロジェクト名
  site.baseUrl … https://<ユーザー名>.github.io
  site.base … /<リポジトリ名>（リポジトリ名を <ユーザー名>.github.io にした場合は /）
 3. Settings → Pages → Source を「GitHub Actions」に切り替え
...
code:dev.sh
 ...
 npm run fetch
 npm run dev
ブラウザで http://localhost:4321/ が開きます。デプロイ手順は [Deploy] を参照。

[** ページを公開する]
Cosense 側で、公開したいページに `#publish` を付けます。最下行にタグだけの行を置くのが推奨です。`#draft` を付けたページは公開されません。
...
[** テンプレートから作った場合]
[https://github.com/shinyaoguri/cosense-theme-default cosense-theme-default] から作ったリポジトリは .github/workflows/build.yml を同梱済みなので deploy init は不要です。Settings → Pages → Source を GitHub Actions にするだけで動きます。
